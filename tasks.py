...
def docker_compose(context, command, **kwargs):
    ...
    if (
        context.nautobot_golden_config.ephemeral_ports
        and context.nautobot_golden_config.compose_files == ORIGINAL_COMPOSE_FILES
    ):
        compose_file_path = os.path.join(
            context.nautobot_golden_config.compose_dir, "docker-compose.ephemeral-ports.yml"
        )
        compose_command_tokens.append(f' -f "{compose_file_path}"')
...
def _ensure_creds_env_file(context):
    ...
    if not os.path.exists(
        os.path.join(context.nautobot_golden_config.compose_dir, "creds.env")
    ):
        ...
        shutil.copy(
            os.path.join(
                context.nautobot_golden_config.compose_dir, "creds.example.env"
            ),
            os.path.join(context.nautobot_golden_config.compose_dir, "creds.env"),
        )
...
def djhtml(context, check=False):
    ...
    command = "djhtml -t 4 nautobot_golden_config/templates/"
